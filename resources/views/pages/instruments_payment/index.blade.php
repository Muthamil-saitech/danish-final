...
<section class="main-content-wrapper">
    ...
    <section class="content-header">
        <div class="row">
            ...
            <div class="col-md-6 text-end">
                <h5 class="mb-0">Total Instrument Payments: {{ isset($total_io) ? $total_io : '0' }} </h5>
            </div>
        </div>
    </section>
                        ...
                        @foreach ($obj as $value)
                            @foreach ($value->details as $detail)
                                <tr>
                                    ...
                                    <td class="text-end">
                                        @if($detail->paymentInvoice->due_amount!=0.00)
                                        <a class="button-success" id="partnerInsDueModal" data-bs-toggle="modal" data-io_detail_id="{{ $detail->id }}" data-tot_amount="{{ $detail->paymentInvoice->amount }}" data-paid_amount="{{ $detail->paymentInvoice->paid_amount }}" data-due_amount="{{ $detail->paymentInvoice->due_amount }}" data-bs-target="#partnerInsDue" data-bs-toggle="tooltip" data-bs-placement="top" title="Amount Entry"><i class="fa fa-money-bill"></i></a>
                                        @endif
                                        <a href="{{ route('instrument-payment-view', encrypt_decrypt($detail->id, 'encrypt')) }}" class="button-info" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('index.view_details')"><i class="fa fa-eye"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        @endforeach
    ...
    <div class="modal fade" id="partnerInsDue" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel">Instrument Payment Entry</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true"><i data-feather="x"></i></span>
                    </button>
                </div>
                <form id="instrument_payment_form" action="{{ route('instrument-pay-entry') }}" method="post" enctype="multipart/form-data">
                    @csrf
                        ...
                        <div class="row">
                            <div class="col-sm-12 col-md-4 mb-2">
                                <div class="form-group">
                                    ...
                                    <input type="hidden" class="form-control" name="io_detail_id" id="io_detail_id">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn bg-blue-btn instrument_payment_submit"><iconify-icon icon="solar:check-circle-broken"></iconify-icon> @lang('index.submit')</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    $(document).on("click", "#partnerInsDueModal", function(e) {
        e.preventDefault();
        var io_detail_id = $(this).data('io_detail_id');
        $('#io_detail_id').val(io_detail_id);
    ...
    $('.instrument_payment_submit').on('click', function(e) {
        e.preventDefault();
        ...
        $("#instrument_payment_form").submit();
    });
</script>
@endsection