                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @if((!$value->inspect_approval) || (is_object($value->inspect_approval) && $value->inspect_approval->status < 2))
                                    ...
                                    data-id="{{ $value->id }}" data-drawer_id="{{ $value->drawer_id }}"><i class="fa-regular fa-circle-check" data-bs-toggle="tooltip" data-bs-placement="top" title="Inspection Dimension"></i></a>
                                    @endif
                            </td>
                        </tr>
                        @endforeach
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('inspect.updateInspectionDimension') }}" id="inspect_dimension" method="post">
                    ...
                    <div class="modal-body">
                        ...
                        <input type="hidden" name="drawer_id" class="drawer_id">
...
<script>
    ...
    $(document).on("click", ".inspection_dimension", function() {
        ...
        let drawer_id = $(this).data("drawer_id");
        $(".drawer_id").val(drawer_id);
        $.ajax({
                    ...
                    response.forEach((item) => {
                        if (item.di_param) {
                            dimensionRows += `
                                    <tr>
                                        <td>${diIndex++}</td>
                                        <td><input type="hidden" name="inspect_id[]" value="${item.id}">${item.di_param}</td>
                                        <td>${item.di_spec}</td>
                        ...
                        if (item.ap_param) {
                            appearanceRows += `
                                    <tr>
                                        <td>${apIndex++}</td>
                                        <td><input type="hidden" name="ap_inspect_id[]" value="${item.id}">${item.ap_param}</td>
                                        <td>${item.ap_spec || ''}</td>
