...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        $total = [
            ...
            'partner' => Partner::where('del_status','Live')->count(),
            'instrument' => Instrument::where('del_status','Live')->count(),
        ];
        ...
        $customer_io = CustomerIO::with('details')->where('del_status', "Live")->orderBy('id', 'DESC')->get();
        $partner_io = PartnerIO::with('details')->where('del_status', "Live")->orderBy('id', 'DESC')->get();
        return view('pages.dashboard', compact('total', 'running_production', 'running_order', 'lowRawMaterialStocks', 'supplierPayments', 'customerPayments', 'closeToFinishProducts', 'percentage_product', 'percentage_rm', 'percentage_supplier', 'percentage_customer', 'mergedProducts','customer_io','partner_io'));
    }
    /**
     * Handle an incoming request.
     *
