...
class StockController extends Controller
{
    ...
    public function rawMaterialStockCheckByMaterial(Request $request)
    {
        ...
        if (!empty($rm_id)) {
            foreach ($rm_id as $key => $value) {
                ...
                if($stk_mat_type=="1") {
                    $ms = MaterialStock::where('mat_id', $rm->id)->where('mat_cat_id', $rm->category)->where('owner_type',$stk_mat_type)->where('supplier_id',$supplier_id)->where('del_status', 'Live')->first();
                } else {
                    $ms = MaterialStock::where('mat_id', $rm->id)->where('mat_cat_id', $rm->category)->where('owner_type',$stk_mat_type)->where('customer_id',$selected_customer_id)->where('del_status', 'Live')->first();
                }
                // dd($ms);
                $consumptionQuantity = $quantity[$key];
                $stock[] = [
                    'id' => $rm->id,
                    'mat_cat_id' => $rm->category,
                    'mat_type_id' => $rm->mat_type_id,
                    'name' => $rm->name,
    ...
    public function checkSingleMaterialStock(Request $request)
    {
        ...
        if (!$order_material) {
            $order_material = CustomerOrderDetails::where('id', $customer_order_id)
                ->where('product_id', $fproduct_id)
                ->where('del_status', 'Live')
                ->first();
        }
        ...
        if($owner_type=="1") {
            ...
            $logsQuery = DB::table('tbl_stock_adjust_logs as logs')
                ...
                ->where('logs.customer_reorder_id', $customer_reorder_id ?? 0)
                ->where('ms.mat_id', $order_material->raw_material_id ?? $order_material->mat_id)
