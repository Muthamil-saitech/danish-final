...
class InspectionGeneratorController extends Controller
{
    ...
    public function show($id) {
        ...
        $inspection = DrawingParameter::where('drawing_id',$manufacture->drawer_id)->where('del_status', 'Live')->get();
        ...
        $productQty = $manufacture->product_quantity ?? 0;
        $columnsPerPage = 13;
        $totalPages = (int) ceil($productQty / $columnsPerPage);
        return view('pages.inspection.inspectionGenerateDetail', compact('title', 'manufacture','finishProduct','material','di_inspect_dimensions','ap_inspect_dimensions','inspection_approval','manage_users','latest_form','material_stock','inspection','columnsPerPage','totalPages'));
    }
    public function printInspectionDetail($id)
    {
        ...
        $title = 'Inspection Report';
        $columnsPerPage = 13;
        $totalPages = (int) ceil(($manufacture->product_quantity ?? 0) / $columnsPerPage);
        return view('pages.inspection.printInspectionReport', compact('title', 'manufacture', 'finishProduct', 'material', 'inspection', 'di_inspect_dimensions', 'ap_inspect_dimensions', 'inspection_approval', 'manage_users', 'material_stock','columnsPerPage','totalPages'));
    }
    ...
    {
        $manufacture = \App\Manufacture::where('id', $request->id)->whereIn('manufacture_status', ['draft', 'inProgress', 'done'])->where('del_status', 'Live')->first();
        // $material = RawMaterial::where('id', $manufacture->rawMaterials[0]->rmaterials_id)->where('del_status', 'Live')->first();
       ...
       $inspectionParams = DrawingParameter::where('drawing_id', $manufacture->drawer_id)->where('del_status', 'Live')->get();
    ...
    public function updateInspectionDimension(Request $request)
    {
        ...
        $drawer_id = $request->get('drawer_id');
        ...
        $merged = [];
        foreach ($di_inspect_ids as $i => $param_id) {
            $merged[$param_id] = [
                'inspect_id'        => $param_id,
        ...
        foreach ($ap_inspect_ids as $i => $param_id) {
        ...
        foreach ($merged as $row) {
            ...
            $inspect_dimension->drawer_id = $drawer_id;
