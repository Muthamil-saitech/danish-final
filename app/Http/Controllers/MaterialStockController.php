...
class MaterialStockController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $obj->dc_date = !empty($request->date) ? date('Y-m-d', strtotime($request->date)) : null;
    ...
    public function material_stock_return($id) {
        $title =  'Material Return Entry';
        $stock_id = encrypt_decrypt($id, 'decrypt');
        $material_stock = MaterialStock::where('id', $stock_id)->where('del_status', 'Live')->first();
        if ($material_stock) {
            $material = RawMaterial::where('id', $material_stock->mat_id)->where('del_status', 'Live')->first();
        ...
        $obj = $material_stock;
        $stock_return = StockReturnEntry::where('stock_id', $stock_id)->first();
        $material_types = MaterialType::where('del_status','Live')->orderBy('id','DESC')->get();
        $mat_categories = RawMaterialCategory::where('del_status', 'Live')->orderBy('id', 'DESC')->get();
        $materials = RawMaterial::where('del_status', 'Live')->orderBy('id', 'DESC')->get();
        return view('pages.material_stock.materialReturn', compact('title', 'obj', 'material','material_types','mat_categories','materials','stock_return'));
    }
    public function printMaterialReturn($id) {
        $title = 'Material Return';
        $material_stock = MaterialStock::where('id', $id)->where('del_status', 'Live')->first();
        if ($material_stock) {
            $material = RawMaterial::where('id', $material_stock->mat_id)->where('del_status', 'Live')->first();
        } else {
            $material = null;
        ...
        $obj = $material_stock;
        $stock_return = StockReturnEntry::where('stock_id', $id)->first();
        return view('pages.material_stock.printMaterialReturn', compact('title', 'obj', 'material','stock_return'));
    }
    public function downloadMaterialReturn($id)
    {
        ...
        $obj = $material_stock;
        $pdf = Pdf::loadView('pages.material_stock.printMaterialReturn', compact('title', 'obj', 'material'))->setPaper('a4', 'landscape');
        return $pdf->download($obj->reference_no.'_'.$obj->line_item_no.'.pdf');
    }
    public function stockReturn(Request $request) {
        // dd($request->all());
        $stock_id = $request->stock_id;
        $mat_type = $request->mat_type;
        $mat_cat_id = $request->mat_cat_id;
        $mat_id = $request->mat_id;
        $reference_no = $request->reference_no;
        $line_item_no = $request->line_item_no;
        $float_stock = $request->float_stock;
        $qty = $request->qty;
        $obj = new StockReturnEntry();
        $obj->stock_id = $stock_id;
        $obj->mat_type = $mat_type;
        $obj->mat_cat_id = $mat_cat_id;
        $obj->mat_id = $mat_id;
        $obj->reference_no = $reference_no;
        $obj->line_item_no = $line_item_no;
