...
use App\Mstages;
use App\Providers\NumberToWordsProvider;
use App\RawMaterial;
...
{
    return CustomerOrderDetails::where('del_status', 'Live')->where('id', $order_id)->first();
}
...
function getStockProduct($reference_no,$line_item_no) {
    $order_id = CustomerOrder::where('reference_no',$reference_no)->where('del_status','Live')->pluck('id');
    $order_product = CustomerOrderDetails::whereIn('customer_order_id',$order_id)->where('line_item_no',$line_item_no)->where('del_status','Live')->pluck('product_id')->first();
    // dd($order_product);
    return $order_product;
}

function getOrderStages($mid) {
    $ids = Mstages::where('manufacture_id', $mid)
        ->where('del_status', 'Live')
        ->pluck('productionstage_id')
        ->toArray();   // <-- important, gives array of IDs

    $stages = \App\ProductionStage::whereIn('id', $ids)->get();
    ...
    foreach ($stages as $stage) {
        $result[] = $i . ' - ' . e($stage->name);
        $i++;
    }

    return implode("\n", $result);
}
function getManufactureStatus($stock_id) {
    $material_item = Mrmitem::where('stock_id',$stock_id)->where('del_status','Live')->first();
    if(isset($material_item) && $material_item->manufacture_id) {
        $manufacture_status = \App\Manufacture::where('id',$material_item->manufacture_id)->pluck('manufacture_status')->first();
    } else {
        $manufacture_status = "draft";
    }
    return $manufacture_status;
}

function getDMYDateFormat($date)
{
    return date('d-m-Y', strtotime($date));
}
function showAmount($amount)
{
    $numberToWords = new NumberToWordsProvider();
    $words = $numberToWords->convert($amount);
