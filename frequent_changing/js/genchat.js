$(function () {
    ...
    $(document).on("click", ".product_scheduling_button", function () {
        ...
        let table = `<tr class="rowCount3" data-row="${j}">
                        ...
                        <td>
                            <select class="form-control" id="manufacture_stage_display_${j}" disabled style="min-width:120px;">
                        ...
                        <td>
                            <input type="text" class="form-control changeableInput" name="task[]" value="${task}" style="min-width:120px;">
                        </td>
                        <td>
                            <select class="form-control changeableInput manufacture_user_id" id="manufacture_user_id_${j}" name="user_id_scheduling[]" style="min-width:120px;">
                                ${user_id}
                            </select>
                        </td>
                        <td>
                            <input type="number" class="form-control" name="task_hours[]" id="task_hours_${j}" value="${task_hours}" style="min-width:120px;">
                        </td>
                        <td>
                            <input type="hidden" name="task_status[]" class="add_task_status">
                            <select class="form-control add_more_task_status changeableInput" id="task_status_${j}" style="min-width:120px;">
                                ${task_status}
                            </select>
                        </td>
                        <td>
                            <input type="text" class="form-control changeableInput pstart_date" name="start_date[]" id="pstart_date" value="${start_date}" style="min-width:120px;">
                        </td>
                        <td>
                            <input type="hidden" name="task_note[]" value="${task_note}">
                            <input type="text" class="form-control changeableInput pcomplete_date" name="complete_date[]" id="pcomplete_date" value="${end_date}" style="min-width:120px;">
                            <p class="text-danger end_date_error d-none"></p>
                        </td>
                        <td>
                            <input type="number" class="form-control" name="complete_hours[]" id="complete_hours_${j}" value="0" readonly placeholder="Minutes" style="min-width:120px;">
                        </td>
                        <td>
                            <input type="text" class="form-control" name="work_centre[]" id="work_centre_${j}" value="" readonly placeholder="Work Centre" style="min-width:120px;">
                        </td>
                        <td class="text-end">
        ...
        $.ajax({
            ...
            success: function (data) { 
                if (!data || data.length === 0) {
                    // If data is empty, remove restriction
                    $('#qc_start_date').datepicker('setStartDate', null);
                    console.log("No restriction on start date");
                    } else {
                    // If data is not empty, set start date as needed
                    $('#qc_start_date').datepicker('setStartDate', data);
                    console.log("Start date restricted to:", data);
                }      
            },
            error: function () {
            }
