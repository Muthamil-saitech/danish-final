$(document).ready(function () {
  ...
  $(document).on("click", "#pr_go", function (e) {
    ...
    if (!Number($(".product_quantity").val())) {
    ...
    } else if(!$("#drawer_no").val()) {
      let hidden_alert = $("#hidden_alert").val();
      let hidden_cancel = $("#hidden_cancel").val();
      let hidden_ok = $("#hidden_ok").val();
      swal({
        title: hidden_alert + "!",
        text: "Drawing No Field is required",
        cancelButtonText: hidden_cancel,
        confirmButtonText: hidden_ok,
        confirmButtonColor: "#3c8dbc",
      });
      ...
      let customer_reorder_id = $("#customer_reorder_id").val();
      // console.log("customer_reorder_id",customer_reorder_id);
      ...
      $.ajax({
        ...
        dataType: "json",
        success: function (data) {
          if (data.delivery_date && data.po_date) {
            ...
            let podateObj = new Date(data.po_date);
            let poday = String(podateObj.getDate()).padStart(2, '0');
            let pomonth = String(podateObj.getMonth() + 1).padStart(2, '0');
            let poyear = podateObj.getFullYear();
            let formattedPoDate = `${poday}-${pomonth}-${poyear}`;
            $("#start_date").val(formattedPoDate);
            $("#po_date_hidden").val(formattedPoDate);
          }
        },
        error: function () {},
      });
      $.ajax({
        ...
        data: {
          ...
          customer_reorder_id: customer_reorder_id,
        },
        dataType: "json",
        success: function (data) {
          if (data.result == true) {
      ...
      $.ajax({
        ...
        data: { id: seperate_drawers[0], fproduct_id: fproduct_id },
        dataType: "json",
        success: function (data) {
      ...
      success: function (data) {
        ...
        if (data.length === 0) {
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            table +=
              '<input type="hidden" name="rm_id[]" value="' + data[i].id + '"><input type="hidden" name="mat_cat_id[]" value="' + data[i].mat_cat_id + '"><input type="hidden" name="mat_type_id[]" value="' + data[i].mat_type_id + '">';
            table +=
  ...
  $(document).on('change', '.supplier_select', function() {
    ...
    var uom = $opt.data('uom') !== undefined ? $opt.data('uom') : '';
    $row.find('.stock_id').val(stockId);
    $row.find('.stock').val(stock);
    $row.find('.show_stock span').text(stock + ' ' + uom);
    $row.find('.show_uom span').text(uom);
  ...
  $(document).on("click", "#fprmaterial", function (e) {
    ...
    $(".add_trm").append(
      "<tr>" +
        ...
        ram_hidden +
        "</select></td>" +
        '<td><div class="input-group"><input type="text" data-countid="1" tabindex="51" id="qty_1" name="quantity_amount[]" onfocus="this.select();" class="check_required form-control input_aligning qty_c cal_row" value="" placeholder="Consumption"><span class="input-group-text rmhunit">Piece</span></div></td>' +
  ...
  $(document).on("submit", "#manufacture_form", function (e) {
    ...
    if (quantity == "") {
      status = false;
      showErrorMessage("product_quantity", "The Product Quantity field is Required");
    } else {
  ...
  $(document).on("change", "#customer_order_id", function (e) {
    ...
    let customer_order_id = $selected.val();
    let order_type = $selected.data("ordertype"); 
    let reorder_id = $selected.data("reorder_id"); 
    let hidden_base_url = $("#hidden_base_url").val();

    $.ajax({
      type: "POST",
      url: hidden_base_url + "getCustomerOrderProducts",
      data: { id: customer_order_id, order_type: order_type, reorder_id: reorder_id },
      success: function (data) {
        $("#fproduct_id option").remove();
        $("#fproduct_id").append(data);
        $(".select2").select2();
  ...
  $(document).on("change", "#fproduct_id", function (e) {
    ...
    let order_type = $selected.data("order_type"); 
    let reorderid = $selected.data("reorderid"); 
    let customer_order_id = $("#customer_order_id").val();
    let hidden_base_url = $("#hidden_base_url").val();
    $.ajax({
      type: "POST",
      url: hidden_base_url + "getProductQty",
      data: { product_id: product_id, customer_order_id: customer_order_id, order_type: order_type, reorderid: reorderid },
      success: function (data) {
        let response = typeof data === "string" ? JSON.parse(data) : data;
        $("#product_quantity").val(response.quantity);
  ...
  $(document).on("keyup", "#minute_limit, #set_minute_limit", function (e) {
    let max = parseInt($(this).attr("max"));
    let min = parseInt($(this).attr("min"));
