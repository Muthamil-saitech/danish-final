...
jqry(document).ready(function(){
    ...
    if (datatable_name && $("#" + datatable_name).length) {
        ...
        targetTable = "table.material_table";
    } */
    let targetTables = new Set();
    if (datatable_name && $("#" + datatable_name).length) {
        targetTables.add("#" + datatable_name); // e.g., #datatable_2
        // Add all tables with IDs starting with 'datatable_' (unique due to Set)
        $("table[id^='datatable_']").each(function() {
            let tableId = "#" + jqry(this).attr("id");
            if (!jqry.fn.DataTable.isDataTable(tableId)) {
                targetTables.add(tableId);
            }
        });
        ...
        targetTables.forEach(function(tableSelector) {
            if (jqry(tableSelector).length) {
                if (!jqry.fn.DataTable.isDataTable(tableSelector)) {
                    jqry(tableSelector).DataTable({
                        autoWidth: false,
                        ordering: false,
                        paging: true,
                        searching: true,
        ...
        if (targetTable) {
            jqry(targetTable).DataTable({
                autoWidth: false,
                ordering: false, // Currently disabled
