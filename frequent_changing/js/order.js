$(document).ready(function () {
  ...
  function handlePOTypeChange() {
    const selectedType = $('input[name="po_type"]:checked').val();
    $('#po_type_hidden').val(selectedType);
    ...
    $('input[name="po_type"]').attr("disabled", true);
  }
  $(document).on("change", 'input[name="po_type"]', function () {
    handlePOTypeChange();
  });
  ...
  $(document).on("focusin", ".rquantity_c, .pquantity_c, .sale_price_c", function () {
    // Store the current value before the user changes it
    $(this).data("old-value", $(this).val());
  });
  $(document).on("change", ".rquantity_c, .pquantity_c, .sale_price_c", function () {
    const oldValue = $(this).data("old-value") || "";
    const newValue = $(this).val();
    const id = $(this).attr("id");

    // Derive field type and index
    const fieldType = id.split("_")[0]; // e.g., 'rquantity' or 'pquantity' or 'sale'
    const index = id.split("_")[1];     // e.g., '1', '2', etc.

    // Hidden input and label IDs
    const hiddenId = `#old_${id}`;
    const labelId = `#old_${id}_label`;

    // Set the old value in hidden input
    $(hiddenId).val(oldValue);

    // Create readable label
    let labelText = "";
    if (fieldType === "rquantity") labelText = `Old Material Quantity: ${oldValue}`;
    else if (fieldType === "pquantity") labelText = `Old Product Quantity: ${oldValue}`;
    else if (fieldType === "sale") labelText = `Old Unit Price: ${oldValue}`;

    // Show it under the field
    $(labelId).text(labelText);

    // Optional: Add a subtle fade effect for visibility
    $(labelId).fadeIn(200);
  });
  ...
  $(document).on("click", ".order_submit_button", function () {
    ...
    $("select[name='product[]']").each(function () {
      ...
      let select2Container = $(this).next('.select2');
      if (product === "") {
        $(this).addClass("is-invalid");
        select2Container.next(".text-danger").remove();
        select2Container.after(
          '<div class="text-danger mt-1">The Part Name (Part No) field is required</div>'
        );
        hasError = true;
      } else {
        $(this).removeClass("is-invalid");
    ...
    $("select[name='raw_material[]']").each(function () {
      let raw_material = $(this).val();
      let select2Container = $(this).next('.select2');
      if (raw_material === "") {
        $(this).addClass("is-invalid");
        select2Container.next(".text-danger").remove();
        select2Container.after(
          '<div class="text-danger mt-1">The Material Name (Code) field is required</div>'
        );
        hasError = true;
      } else {
        $(this).removeClass("is-invalid");
    ...
    $("input[name='raw_quantity[]']").each(function () {
      let raw_quantity = $(this).val();
      if (raw_quantity === "") {
        $(this).addClass("is-invalid");
        if (!$(this).next(".text-danger").length) {
          $(this).after(
            '<div class="text-danger">The Material Quantity is required</div>'
          );
        }
        hasError = true;
      } else {
    ...
    $("input[name='prod_quantity[]']").each(function () {
      let prod_quantity = $(this).val();
      if (prod_quantity === "") {
        $(this).addClass("is-invalid");
        if (!$(this).next(".text-danger").length) {
          $(this).after(
            '<div class="text-danger">The Product Quantity is required</div>'
          );
        }
        hasError = true;
      } else {
    ...
    $("input[name='sale_price[]']").each(function () {
      ...
      let parentGroup = $(this).closest(".input-group").parent();
      if (sale_price === "") {
        $(this).addClass("is-invalid");
        parentGroup.find(".text-danger").remove();
        parentGroup.append(
          '<div class="text-danger mt-1">The Unit Price is required</div>'
        );
        hasError = true;
      } else {
        $(this).removeClass("is-invalid");
    ...
    $("input[name='line_item_no[]']").each(function () {
      ...
      let existingLineItems = $("input[name='line_item_no[]']")
        ...
        .get();
      $(this).removeClass("is-invalid");
      $(this).next(".text-danger, .duplicate-msg").remove();
      if (lineNo === "") {
        $(this).addClass("is-invalid");
        $(this).after('<div class="text-danger">The PO Line Item No field is required</div>');
        hasError = true;
        return;
      }
      if (existingLineItems.includes(lineNo)) {
        $(this).addClass("is-invalid");
        $(this).after('<div class="text-danger duplicate-msg">PO Line Item No already exists.</div>');
        hasError = true;
      }
    });
    if (hasError) {
