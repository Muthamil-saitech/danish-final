$(document).ready(function () {
    ...
    function cal_row() {
        ...
        let final_subtotal = row_tatal_total + other_amount - totalDiscount;
        let tax_percent = $("#tax_percent").val();
        let tax_amount = 0;
        if(tax_percent == "") {
            tax_amount;
        } else {
            tax_amount = final_subtotal * (parseFloat($.trim(tax_percent)) / 100);
        }
        $("#tax_amount").val(tax_amount);
        $("#grand_total").val(
            (final_subtotal + tax_amount).toFixed(2)
        );
        let due = row_tatal_total - paid + other_amount - totalDiscount + tax_amount;
    ...
    $("#customer_quote_form").submit(function (e) {
        ...
        if (order_type == "Quotation" && (discount == "" || discount == "0" || discount == 0)) {
            status = false;
            showErrorMessage("discount", "The Without Material field is required");
        } else {
            $("#discount").removeClass("is-invalid");
            $("#discount").closest("div").find(".text-danger").addClass("d-none");
        }
