...
(function ($) {
    ...
    $(document).on("change", "#inp_ref_no", function (e) {
        let selected = $(this).val();
        let split = selected.split('|');
        let split_ref = split[0].split('/');
    ...
    $("#material_stock_form").submit(function () {
        ...
        if(dc_no == "" && owner_type == "2" || owner_type == "") {
            status = false;
        ...
        if(dc_date == "" && owner_type == "2" || owner_type == "") {
            status = false;
        ...
        if(mat_id == "") {
            status = false;
            showErrorMessage("mat_id", "The Material Name field is required");
        }else{
        ...
        if(owner_type == "2" && customer_id == null || customer_id == "") {
            status = false;
